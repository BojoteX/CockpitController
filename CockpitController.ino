...
/*
// DCS-BIOS Specific
DcsBios::IntegerBuffer* ledBuffers[NUM_DCS_LEDS];

// Efficient binary search lookup
const DcsLed* getLedByAddress(uint16_t address) {
  int low = 0;
  int high = NUM_DCS_LEDS - 1;

  while (low <= high) {
    int mid = (low + high) / 2;
    uint16_t midAddress = dcsLeds[mid].address;
...
// Callback to handle LED state updates
void handleLedChange(uint16_t address, uint16_t mask, uint8_t shift, unsigned int newValue) {
  const DcsLed* led = getLedByAddress(address);
  if (led) {
    bool state = ((newValue & mask) >> shift) != 0;
    setLED(led->label, state, state ? 255 : 0);
  }
}

void setupDcsBiosBuffers() {
  for (size_t i = 0; i < NUM_DCS_LEDS; i++) {
    ledBuffers[i] = new DcsBios::IntegerBuffer(
      dcsLeds[i].address, dcsLeds[i].mask, dcsLeds[i].shift,
      ledCallbacks[i] // Use autogenerated callback array
    );
  }
}
*/
